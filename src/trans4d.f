...
*** Obtain parameters defining crustal motion model
      ...
      COMMON /CONST/ A,F,E2,EPS,AF,PI,TWOPI,RHOSEC
      COMMON /TIMREF/ ITREF
      COMMON /CURRENTGRID/ IGRID(NUMGRD), IACTIVE
...
C***  specify initial storage grid ID
      IACTIVE = 0
      IGRID(1) = 1
      IGRID(2) = 1
      IGRID(3) = 2
...
*** Region 9 is the island of Hawaii
*** Region 10 is the Bering region
...
C
      ...
      COMMON /CONST/ A,F,E2,EPS,AF,PI,TWOPI,RHOSEC
      COMMON /FILES/ LUIN, LUOUT, I1,I2,I3,I4,I5,I6
      COMMON /VGRID/ B(750000)
      COMMON /SGRID/ C(750000)
      DIMENSION WEI(2,2), VEL(2,2,3), STDEV(2,2,3)

c      WRITE (6, 1001) JREGN
c 1001 FORMAT( 'JREGN = ', I6)
...
***********************************
     ...
     *       ICNTX(NUMGRD), ICNTY(NUMGRD), NBASE(NUMGRD)        
      ...
      if(IGRID(jregn) .eq. IACTIVE) return
      
      if(IGRID(jregn) .eq. 1) then
          IACTIVE = 1
          
          open(i4, file = 'GRID-SanAnd.txt', status = 'old')
          k = 1
          nbase(k) = 0
          call getstats(k)
          idim = icntx(k)
          jdim = icnty(k)
          kdim1 = (idim + 1)*(jdim + 1)
          call ReadVelocities(k, kdim1)
          CLOSE(I4, status = 'keep')
          
          open(i4, file = 'GRID-WCONUS.txt', status = 'old')
          k = 2
          nbase(k) = nbase(1) + 3*kdim1
          call getstats(k)
          idim = icntx(k)
          jdim = icnty(k)
          kdim2 = (idim + 1)*(jdim + 1)
          call ReadVelocities(k, kdim2)
          CLOSE(I4, status = 'keep')
    
      elseif(IGRID(jregn) .eq. 2) then
          IACTIVE = 2
          open(i4, file = 'GRID-SEUS.txt', status = 'old')
          k = 3
          nbase(k) = 0
          call getstats(k)
          idim = icntx(k)
          jdim = icnty(k)
          kdim3 = (idim + 1)*(jdim + 1)
          call ReadVelocities(k, kdim3)
          close(I4, status = 'keep')
          
          open(I4, file = 'GRID-NEUS.txt', status = 'old') 
          k = 4
          nbase(k) = nbase(3) + 3*kdim3
          call getstats(k)
          idim = icntx(k)
          jdim = icnty(k)
          kdim4 = (idim + 1)*(jdim + 1)
          call ReadVelocities(k, kdim4)
          close(I4, status = 'keep')
     
       elseif(IGRID(jregn) .eq. 3) then
          IACTIVE = 3

          open(i4, file = 'GRID-Alaska.txt', status = 'old')
          k = 5
          nbase(k) = 0
          call getstats(k)
          idim = icntx(k)
          jdim = icnty(k)
          kdim5 = (idim + 1)*(jdim + 1)
          call ReadVelocities(k, kdim5)
          close(I4, status = 'keep')
          
          open(i4, file = 'GRID-Vancouver.txt', status = 'old')
          k = 6
          nbase(k) = nbase(5) + 3*kdim5
          call getstats(k)
          idim = icntx(k)
          jdim = icnty(k)
          kdim6 = (idim + 1)*(jdim + 1)
          call ReadVelocities(k, kdim6)
          close(I4,status = 'keep')
          
          open(i4, file = 'GRID-Canada.txt', status = 'old')
          k = 7
          nbase(k) = nbase(6) + 3*kdim6
          call getstats(k)
          idim = icntx(k)
          jdim = icnty(k)
          kdim7 = (idim + 1)*(jdim + 1)
          call ReadVelocities(k, kdim7)
          close(I4, status = 'keep')

        elseif(IGRID(jregn) .eq. 4) then
           IACTIVE = 4
           
           open(i4, file = 'GRID-CARIB.txt', status = 'old')
           k = 8
           nbase(k) = 0
           call getstats(k)
           idim = icntx(k)
           jdim = icnty(k)
           kdim8 = (idim + 1)*(jdim + 1)
           call ReadVelocities(k, kdim8)
           close(I4, status = 'keep')
           
        elseif(IGRID(jregn) .eq. 5) then
           IACTIVE = 5
           
           open(I4, file = 'GRID-Hawaii.txt', status = 'old')
           k = 9
           nbase(k) = 0
           call getstats(k)
c            write(I2,1001) grdlx(9), grdux(9)
c  1001      format('grdlx(9) = ', f10.5,
c     1            'grdux(9) = ', f10.5)
           idim = icntx(k)
           jdim = icnty(k)
           kdim9 = (idim + 1)*(jdim + 1)
           call ReadVelocities(k, kdim9)
           close(I4, status = 'keep')
           
        elseif(IGRID(jregn) .eq. 6) then
           IACTIVE = 6

          open(I4, file = 'GRID-Bering.txt', status = 'old')
           k = 10
           nbase(k) = 0
           call getstats(k)
c            write(I2,1001) grdlx(9), grdux(9)
...
c     1            'grdux(9) = ', f10.5)
           idim = icntx(k)
           jdim = icnty(k)
           kdim10 = (idim + 1)*(jdim + 1)
           call ReadVelocities(k, kdim10)
           close(I4, status = 'keep')     
      else 
         write(LUOUT,*) ' Improper region in subroutine GETGRID '
         STOP
      endif 
...
****************************************** 
      SUBROUTINE ReadVelocities(k, kdimx)
      
      implicit double precision (a-h, o-z)
      implicit integer (i-n)
      
c       parameter (numgrd = 10)
      
c       common /CURRENTGRID/ IGRID(numgrd), IACTIVE
      
      common /FILES/ LUIN,LUOUT,I1,I2,I3,I4,I5,I6
c      common /CDGRID/ GRDLX(NUMGRD), GRDUX(NUMGRD),
c      *       GRDLY(NUMGRD), GRDUY(NUMGRD),
c     *       ICNTX(NUMGRD), ICNTY(NUMGRD), NBASE(NUMGRD)
      common /VGRID/ B(750000) 
      common /SGRID/ C(750000)
       
      do 100 L = 1, kdimx
          read(i4,50) i, j, vn,sn,ve,se,vu,SU
   50     format(2I3, 26x, 6f8.2)
c         write(i2,50) i,j,vn,sn,ve,se,vu,su
          index = iungrd(k,i,j,1)
          index1 = index + 1
          index2 = index + 2
          b(index) = vn
          c(index) = SN
...
***    The resulting velocities--VX, VY, and VZ--will be in meters/yr
      ...
      IMPLICIT DOUBLE PRECISION (A-H, O-Z)
      IMPLICIT INTEGER*4 (I-N)
      parameter (numplate = 11)
      
      DIMENSION WX(numplate), WY(numplate), WZ(numplate)
      COMMON /FILES/ LUIN, LUOUT, I1, I2, I3, I4, I5, I6

*** IPLATE = 1 --> North America (Ding et al. 2019, JGR-Solid Earth)
***          2 --> Caribbean (Snay & Saleh, in preparation)
***          3 --> Pacific (Altamimi et al. 2017, JGR-Solid Earth)
***          4 --> Juan de Fuca (DeMets et al. 2010, Geophys. J. Intl, vol 181)
***          5 --> Cocos (DeMets et al. 2010, Geophys. J. Intl, vol. 181)
***          6 --> Mariana (Snay, 2003, SALIS, Vol 63)
***          7 --> Philippine Sea (Kreemer et al. 2014, Geochem. Geophys. Geosyst., vol. 15)
***          8 --> South America (Altamimi et al. 2017, JGR-Solid Earth)
***          9 --> Nazca (Altamimi et al. 2017, JGR-Solid Earth)
***         10 --> Panama (Kreemer et al. 2014, Geochem. Geophys. Geosyst., vol 15)
***         11 --> North Andes (Mora-Paez, 2018, J. South Amer. Earth Science)

      DATA WX /0.2668D-9, -0.188D-9,-1.983D-9, 
     1         6.636D-9, -10.380D-9,-0.097D-9,
     2         9.221D-9,  -1.309D-9,-1.614D-9, 
     3         2.088D-9,  -1.964D-9 /
      DATA WY /-3.3677D-9, -4.730D-9, 5.076D-9,
     1         11.761D-9,-14.901D-9, 0.509D-9,
     2         -4.963D-9, -1.459D-9,-7.486D-9, 
     3        -23.037D-9, -1.518D-9 /
      DATA WZ /-0.2956D-9, 2.963D-9,-10.516D-9, 
...
*********************      
      SUBROUTINE OKADAW(PSI,ETA,Q,SDIP,CDIP,RATIO,
     1   TWOPI,VERT,U1SS,U2SS,U3SS,U1DS,U2DS,U3DS)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      IMPLICIT INTEGER*4 (I-N)
      LOGICAL VERT   
     ...
     1    + YBAR*Q/((R + DBAR)*(R + DBAR)) - DLOG(R + ETA))
      ...
      ELSE
         ...
         ELSE
            F5 = 2.D0*RATIO*
     1       DATAN((ETA*(X+Q*CDIP)+X*(R+X)*SDIP)/(PSI*(R+X)*CDIP))/CDIP
         ENDIF
...
C
      A=  INT(IYRP*0.01D0)
      B=  INT(2 - A + DINT( A*0.25D0 ))
      C=  INT(365.25D0*IYRP)
      D=  INT(30.6001D0*(IMOP + 1))
   ...
   55 FORMAT(' ************************************************'/
     ...
     2   ' positions and displacements.  '/)
      ...
      IF(IOPT .GE. 1 .AND . IOPT .LE. numref) THEN
          WRITE(I2,57) frame1
   57     FORMAT(' DISPLACEMENTS IN METERS RELATIVE TO ', a28)
      ELSE
          WRITE(LUOUT,70)
...
c            zlat = DBLE(LAT)/3600.d0
c            zlon = 360.d0 - DBLE(LON)/3600.d0
c            write(i2,295) zlon, zlat, de, dn, zero,zero,zero
c 295        format(f11.5,1x,f10.5,1x,f6.3,1x,f6.3,3(1x,f4.1))
          ENDIF
	  GO TO 290
  296     WRITE(LUOUT,297)
  297     FORMAT(' ***********************************'/
...
*** Choosing input format for locations where velocities are to be predicted
 ...
 1061  Format(
  ...
  120      J = J + 1
       ...
       ZLON = 360.0d0 - DBLE(LON)/3600.0D0
	   YLON = DBLE(LON)/RHOSEC
           CALL TODMSS(YLON,LOND,LONM,SLON,ISIGN)
	   LONDIR = 'W'
	   IF (ISIGN .eq. -1) LONDIR = 'E'
...
c             stdhor = dmax1(sn,se)

*** code to create vectors to be plotted
c 	       IF (TOTVEL .GE. 1.1D0) THEN
c 	          TVE = VE/TOTVEL
c  	          TVN = VN/TOTVEL
c                 write(I2,129) zlon,zlat,tve,tvn,dummy,dummy,
c     1                      dummy,name4
c  129             format(f10.4,f10.4,5(1x,f7.2),1x,a4)
c    	    ENDIF
...
c          if(dabs(vu) .gt. 0.001d0) then
c             write(I2,129) zlon, zlat, vu 
c         endif  
c              write(I2,129) zlon, zlat, stdhor
c              write(I2,129) zlon, zlat, su 
c  129      format(f10.4,1x,f10.4,1x,f7.2)
c               if (stdhor .lt. 2.0d0) then
c                 write(I2,129) zlon, zlat, stdhor
c               endif
c             if (su .ge. 0.001d0) then
c                write(I2, 129) zlon, zlat, su
c             endif
 
*** code to create grid of zero velocities
c      namer= 'PV ZZZZZ'
c      title = 'RESID'
c      info = ' XXXX:XXX XXXX:XXX O'
c      vel = 0.0d0
c      sigma = 9.99d0
c      write(I2,129) namer, title, zlat, wlon, vel, sigma,
c    1      vel, sigma, vel, sigma, info 
c 129 format(a8,12x,a5,1x,f10.6,1x,f11.6,6f7.2,a20)
*** end of temporary code
...
*** start of test code
c             xlon = 360.d0 - xlon
c             read(name50,102) vnobs, sigvn, veobs, sigve
c 102         format(5x,4f7.2)
c             residn = vnobs - vn
c             reside = veobs - ve
c             corr = 0.d0
c             write(i2,101) xlon, xlat,reside,residn,sigve,sigvn,
c    1                      corr,name50
c 101         format(f11.6, 1x, f10.6, 1x, 5f7.2,1x,a4)
*** end of test code 
              IF(PVOUT .EQ. 'Y') then
                call PVPRNT(LATD,LATM,SLAT,LOND,LONM,SLON,VN,VE,VU)
              ENDIF
...
c  read a PV-file and create a new PV-file with residual velocities
         izero = 0
         zero = 0.00d0
         write(luout, 400)
         read(luin, 410) NAMEBB
         open(I1, file = NAMEBB, status = 'OLD')
         eht = 0.0d0
  470    read(i1,480,end = 590) record
  480    format(a110)
         read(record,490) xlat,xlon1,vnobs,
     1        sign,veobs,sige,vuobs,sigu
  490    format(26x,f10.6,1x,f11.6,6f7.2)
         xlon =360.d0 - xlon1
         if (xlat .le. 24.0d0 .and. xlat .ge. 6.0d0 .and.
     1       xlon .le. 95.0d0 .and. xlon .ge. 57.0d0
     1       .and. sign .le. 2.00d0
     1       .and. sige .le. 2.00d0)then
c           write(i2, 491) xlon1, xlat, vuobs
c  491      format(f12.6, f12.6, f8.2)         
c           write(i2, '(a110)') record
           
              ylat = (xlat*3600.d0)/rhosec
              ylon = (xlon*3600.d0)/rhosec
          call gtovel(ylat,ylon,eht,vn,ve,vu,
     1      vx,vy,vz,jregn,iopt,sn,se,su,sx,sy,sz)
         if (jregn .eq. 0) then
            write(i2,500) record(1:26)
  500       format(' OUT of REGION',5x,a26)
         
          else
            vtotal = dsqrt(ve*ve + vn*vn)
            if (vtotal .ge. 1.d0) then
c               vn = vn/vtotal
c               ve = ve/vtotal
             write(i2,505) -xlon,xlat,ve,vn,zero,zero,zero
  505         format(2f10.4,5f8.2) 
            endif
c               residn = vnobs - vn
c               reside = veobs - ve
c               residu = vuobs - vu
c             write(record(50:55),510) residn
c             write(record(64:69),510) reside
c            write(record(77:83),511) residu
c  510       format(f6.2)
c  511       format(f7.2)
c              write(i2, 512) record
c           write(i2, 512) record, jregn
c  512       format(a110)   
          endif
        endif
        go to 470
  590   close(i1, status = 'keep')     
...
*** involves only frame transfer      
  ...
  150 format(' TRANSFORMING POSITIONS FROM ',A28,' (EPOCH = ',
     1  I2.2,'-',I2.2,'-',I4,' (',F9.4,'))'/26X,
     1  'TO ', A28,' (EPOCH = ',I2.2,'-',I2.2,'-',I4,' (',F9.4,'))'/)
  ...
  170 format (/' ***************************************'/
     ...
     1  '   4...  Transform positions using velocities contained in a'/
     1  '         batch file of deliminated records of the form: '/
     1  '         X,Y,Z,Vx,Vy,Vz,TEXT '/
     1  '         X,Y,Z, are Cartesian coordinates of the stattion '/
     1  '         Vx,Vy,Vz are Cartesian velocities in m/year '/
     1  '         TEXT = Descriptive text (CHARACTER*24) ' /
     1  '         Example: ' /
     1  '  -86682.104,-5394026.861,3391189.647,-0.012,-0.001,-0.001,'
     1  ' SALT '/)
      read(luin,'(a1)',err=602,iostat=ios) option
      if (ios /= 0) goto 602
  ...
  180    call GETPNT(latd, latm, slat, LATDIR, lond, lonm, slon, 
     1        LONDIR, name24, x, y, z, ylat, ylon, eht)
         ...
         if (min1 .eq. min2) then
             ...
             call getvly(ylat,ylon, vx, vy, vz,
     1            vn, ve, vu, vopt, 210) 
             if (vopt .eq. '0') then
...
c         write(luout, *) x,y,z, Vx, Vy, Vz, name24
         ...
         if (min1 .eq. min2) then
            ...
            if(.not.FRMXYZ(x,y,z,ylat,elon,eht)) STOP 666
            if (elon .lt. 0.d0) elon = elon + twopi
            call TOVNEU(ylat,elon,vx,vy,vz,vn,ve,vu)
C  coverting velocities from m/yr to mm/yr
            vnmm = 1000.d0 * vn
            vemm = 1000.d0 * VE 
            vumm = 1000.d0 * vu 
c             write(luout,*) ' vn ve vu = ', vnmm, vemm, vumm
            ylon = -elon
            if(ylon .lt. 0.d0) ylon = ylon + twopi
            call NEWCOR(ylat,ylon,eht,min1,min2,
     1         ylat1,ylon1,eht1,dn,de,du,vnmm,vemm,vumm) 
c            write(luout,*) ' dn de du = ', dn,de,du
            call TOXYZ(ylat1,-ylon1,eht1,x1,y1,z1)
c             write(luout,*) 'x1, y1, z1 = ', x1, y1, z1
          endif      
          call TO_itrf2014(x1,y1,z1,x2,y2,z2,date2, iopt1)
          call from_itrf2014(x2,y2,z2,x3,y3,z3,date2,iopt2)
          call extract_name (name24,iii)
...
*** From ITRF2014 to ITRF2014-PMM for North America
      ...
      ry(10) = 0.0d0
      rz(10) = 0.0d0
      drx(10) = +0.0000550d0 / rhosec
      dry(10) = -0.0006946d0 / rhosec
      drz(10) = -0.0000610d0 / rhosec
      scale(10) = 0.0d-9
      dscale(10) = 0.0d-9
      refepc(10) = 2010.0d0
...
** Write out options for reference frames
      ...
      iframe(4) = 1
      nframe(4) = 'WGS_84(transit)             '
      iframe(5) = 2
      nframe(5) = 'ITRF88                      '
      iframe(6) = 3
      nframe(6) = 'ITRF89                      '
      iframe(7) = 4
      nframe(7) = 'ITRF90/PNEOS_90/NEOS_90     '
      iframe(8) = 5
      nframe(8) = 'ITRF91 or WGS_84(G730)      '
      iframe(9) = 6
      nframe(9) = 'ITRF92                      '
      iframe(10)= 7
      nframe(10)= 'ITRF93                      '
      ...
      nframe(11)= 'ITRF94                      '
      iframe(12)= 8
      nframe(12)= 'ITRF96 or WGS_84(G873)      '
      iframe(13)= 9
      nframe(13)= 'ITRF97 or IGS97             '
      iframe(14)= 11
      nframe(14)= 'ITRF2000/IGS00/WGS_84(G1150)'
      iframe(15)= 14
      nframe(15)= 'ITRF2005 or IGS05           '
      iframe(16)= 15
      nframe(16)= 'ITRF2008/IGS08/WGS_84(G1762)'
      iframe(17)= 16
      nframe(17)= 'ITRF2014 or IGS14           '
      iframe(18)= 21
      nframe(18)= 'ITRF2020 or IGS20           '
      iframe(19)= 10
      nframe(19)= 'NATRF2014                   '
      iframe(20)= 18
      nframe(20)= 'PATRF2014                   '
      ...
      nframe(22)= 'MATRF2014                   '
      iframe(23) = 20
      nframe(23) = 'BERING2014                 '
      
      write(luout, 100)
  100 format(
     1' NATIONAL REFERENCE FRAMES '/
     1'   1...NAD_83(2011/CORS96/2007) (for use near North America) '/
     1'   2...NAD_83(PA11/PACP00)      (for use on the Pacific plate) '/
     1'   3...NAD_83(MA11/MARP00)      (for use on the Mariana plate) '/
     1'                                                   ')
     
      write(luout,101)
  101 format(    
     1' INTERNATIONAL REFERENCE FRAMES '/
     1'   4...WGS_84(transit)    (NAD_83(2011) used) '/
     1'   5...ITRF88 '/
     1'   6...ITRF89 '/
     1'   7...ITRF90 or PNEOS_90 or NEOS90 '/
     1'   8...ITRF91 or WGS_84(G730) '/
     1'   9...ITRF92 '/
     1'  10...ITRF93 '/
     1'  11...ITRF94 or WGS_84(G873) '/
     1'  12...ITRF96 '/
     1'  13...ITRF97 or IGS97 ')
     
      write(luout,102)
  102 format(    
     1'  14...ITRF2000 or IGS00 or IGb00 or WGS_84(G1150) '/
     1'  15...ITRF2005 or IGS05    '/
     1'  16...ITRF2008 or IGS08 or IGb08 or WGS_84(G1762)'/
     1'  17...ITRF2014 or IGS14 '/ 
     1'  18...ITRF2020 or IGS20 '/
     1'                           '/
     1' EXPERIMENTAL REFERENCE FRAMES (NOT FOR OFFICIAL USE) '/
c     1'       Positional coordinates correspond to ITRF2014  '/ 
c     1'  19...NATRF2014 (velocities are relative to preliminary  '/
c     1'       Euler pole parameters for the North American plate '/
     1'  20...PATRF2014 (velocities are relative to preliminary  '/
     1'       Euler pole parameters for the Pacific plate        '/
     1'  21...CATRF2014 (velocities are relative to preliminary  '/
     1'       Euler pole parameters for the Caribbean plate      '/
c     1'  22...MATRF2014 (velocities are relative to preliminary  '/
c     1'       Euler pole parameters for the Mariana plate        '/
     1'  23...BETRF2014 (velocities are relative to preliminary  '/
     1'       Euler pole parameters for the Bering plate         '/
     1     )

   40 read (luin, *,err=50,iostat=ios) iopt
      if (ios /= 0) goto 50
      
      if ( 1 .le. iopt .and. iopt .le. 23) then
	     mframe = nframe(iopt)
	     kopt = iframe(iopt)
      else
	     write (luout,42) 
   42 format ('Improper input,'
     1   ' enter a number between 1 and 23')
	     go to 40
      endif
      return
...
** specified date and reference frame.
      ...
      parameter (numref = 21)
      parameter (nbbdim = 10000)
      CHARACTER*30 OLDBB, NEWBB
      CHARACTER*1  OPT, ANSWER
      character*28 frame1, frame2
      character*10 Trans4D_version   
      LOGICAL TEST
      COMMON /CONST/ A,F,E2,EPS,AF,PI,TWOPI,RHOSEC
...
*** Update G-FILE
  ...
  640          format( ' ***CAUTION: All GPS interstation vectors',
     1                 ' have been transformed to ', a28, ' ***')
...
*** Update G-FILE of blue book
      ...
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      IMPLICIT INTEGER*4 (I-N)
      integer*8 mjd1, mjd0, mjd2
      integer*8 mino, mino1, mino2, min2
...
*** Convert vector from JREF to IOPT frame
            if(jref .eq. 1 .or. iopt .eq. 1) then                        
...
*** Convert GPS vector from IOPT to KOPT reference frame
            IF (KOPT .NE. -1) THEN
              if(iopt .eq. 1 .or. kopt .eq. 1) then
                CALL TRAVEC(DX, DY, DZ, DATE2, IOPT, KOPT)
              else
                call TRAVEC_IERS(dx,dt,dz,dates,iopt,kopt)
              endif
            ELSE
              if(iopt .eq. 1 .or. jref .eq. 1) then
...
c*** the following two lines of code were added on July 20, 2015
         ...
         ELSE
             CALL XtoITRF2014(X,Y,Z,RLAT,RLON,EHT14,DATE,IOPT)
c            CALL XtoITRF2014(X,y,Z,RLAT,RLON,EHT,DATE,IOPT) **error**             
         ENDIF
...
*** Transform velocities from one reference frame to another
  ...
  125 format( ' TRANSFORMING VELOCITIES FROM ', A28, ' TO ', a28//)
  ...
  220    close (i1, status = 'keep')
      elseif (option .eq. '3') then
         ...
         read(luin, '(a)',err=602,iostat=ios) nameif
         open(i1,file=nameif,status='old')
  310    read(i1,311,end=320) space1,glat,glon,
     *                vn,sn,ve,se,vu,su,space2
  311    format(a26,f10.6,2x,f10.6,6f7.2,a20)
        
         ylat = (glat*3600.d0)/rhosec
         elon = (glon*3600.d0)/rhosec
         call TOXYZ(ylat,elon,eht,x,y,z)
         call TOVXYZ(ylat,elon,vn,ve,vu,vx,vy,vz)
         vx1 = vx
         vy1 = vy
         ...
         call TOVNEU(ylat,elon,vx1,vy1,vz1,vn1,ve1,vu1)
c          space2(20:20) = '1'
c         write(i2,311) space1,glat,glon,vn1,sn,
c     1         ve1,se,vu,su,space2   
          glon1 = glon - 360.0d0 
          write(i2,312) glon1, glat, ve1, vn1, zero, zero, zero
  312    format(f11.6,2x,f11.6,2f8.2,3f6.1)   
         go to 310
  320    close(i1, status = 'keep')       
      endif
  500 continue
      close (i2, status = 'keep')
      write(luout,501)
...
***    TEST       output - if (true) then there is an error
      ...
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      IMPLICIT INTEGER*4 (I-N)
      integer*8 MINS, MJD, MJD0, MJD1, IREMDAY
      DIMENSION M(12)
      LOGICAL TEST
...
****
          ...
          IREMDAY = (INT(DATE) - IYEAR)* (MJD1 - MJD0)
          IBEGIN = 0
          ITOTAL = 31
          IF (IREMDAY .LT. ITOTAL) then
              MONTH = 1
              IDAY = IREMDAY - IBEGIN + 1
              CALL IYMDMJ(IYEAR,MONTH,IDAY,MJD)
              MINS = MJD * 24 * 60
              TEST = .FALSE.
              RETURN
...
C              GRID NODES OF GRID K WHERE I,J ARE THE INDICES OF
...
********************************************************      
     ...
     1          ICNTX(NUMGRD), ICNTY(NUMGRD), NBASE(NUMGRD)
      ...
      READ(I4,10) LONMIN, LONMAX, ICNTX(K),
